...
import math
# 提高频率后的信号
r_1 = np.ones(total_steps + P) * 1.0 + 0.5 * np.sin(np.linspace(0, 5*np.pi, total_steps + P))
r_2 = np.ones(total_steps + P) * 2.0 + 0.3 * np.cos(np.linspace(0, 4*np.pi, total_steps + P))

...
delta_u2_min = -5.0
delta_u2_max = 5.0
positive_delta_u1_constrain = 0.3
negative_delta_u1_constrain = -0.3
positive_delta_u2_constrain = 0.3
negative_delta_u2_constrain = -0.3


# 定义代价函数的权重系数
Q1 = np.diag(np.ones(P) * 1.0)  # 对应 err 的权重矩阵
Q2 = np.diag(np.ones(P) * 1.0)  # 对应 err 的权重矩阵
R_delta1 = np.diag(np.ones(M) * 0.00001)  # 对应 U[i+1] - U[i] 的权重矩阵
R_delta2 = np.diag(np.ones(M) * 0.00001)  # 对应 U[i+1] - U[i] 的权重矩阵


# 替换原来的r_1和r_2定义
import math
# 提高频率后的信号
r_1 = np.ones(total_steps + P) * 1.0 + 0.5 * np.sin(np.linspace(0, 10*np.pi, total_steps + P))
r_2 = np.ones(total_steps + P) * 1.0 + 0.3 * np.cos(np.linspace(0, 8*np.pi, total_steps + P))

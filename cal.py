...
R_delta = np.ones(M) * 0.01  # 对应 U[i+1] - U[i] 的权重系数
...
for k in range(total_steps):
    ...
    # 添加 delta_u 的上下界约束
    model.addConstr(U[0] - u_previous >= delta_u_min, name="delta_u_min_0")
    model.addConstr(U[0] - u_previous <= delta_u_max, name="delta_u_max_0")
    for i in range(M - 1):
        model.addConstr(U[i + 1] - U[i] >= delta_u_min, name=f"delta_u_min_{i+1}")
        model.addConstr(U[i + 1] - U[i] <= delta_u_max, name=f"delta_u_max_{i+1}")
